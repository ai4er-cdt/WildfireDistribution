...
def pull_composite_image(START_DATE, END_DATE, AOI):
    """
    Pulls composite image based on these standard levels given by google cloud development script, based on start, end date and AOI
    """
    CLOUD_FILTER = 60
...
def create_latlon_grid(minlat  = 50.77946266, maxlat=  53.04445373, minlon= 22.9446959 , maxlon= 32.08155782):
    ...
    TODO update the increments in the linspace range to deal with inputs of any shape - current workaround, try differnt levels until able to download 
    """
...
def pull_monthly_cloudless_sentinel(years,outdir, coords =( 50.77946266, 53.04445373,22.9446959, 32.08155782) ):
    """
    Download normalized monthly medians of composite cloudless sentinel 2 data for bands 3,4,8 to use in CNN. for all months in years provided.
    Currently normalised using the min = 0 max = 65535 of bands 3,4,8 from Sentinel2 data. using scale 50m for outputs - this is to speed up downloads. 
    ...
    Update to allow input of geometry shape and automatic breakdown. check max min date of sentinel2 data and add checks on valud dates
...
if __name__ == '__main__':
    years = [2016]
    outdir = r'/gws/nopw/j04/bas_climate/projects/WildfireDistribution/cloudless/2016'
    
    if os.path.isdir(outdir) is False:
        os.mkdir(outdir)
        
    pull_monthly_cloudless_sentinel(years, outdir)
