...
def query_sentinel(key_json, project_id, start, end, row, path, cloud=100.):
    ...
    query = client.query("""
        ...
        """.format(r=row,p =path,  s=start, e=end))
...
def download_landsat(row_list, path_list, bands, outdir, cloud):
    ...
    for x in row_list:
        ...
        for y in path_list:
            path = y 
            scene_list = query_sentinel(key_json, project_id, start, end, row, path, cloud)
