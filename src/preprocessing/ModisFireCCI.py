    ...
    * `User Guide
    ...
    def export(self, target_dir):
        ...
        if self.data is None:
            raise ValueError(
                "The data is = None. Try calling 'populate' to assign the object data before making this call."
            )
        ...
        for dataArray in self.data:
            output_name = (
                "PostProcessing_"
                + str(dataArray.date.values)
                + "-"
                + self.filename_glob[1:]
            )
    ...
    def get_single_pixel_fire_proportion(self):
        """Returns the proportion of single-pixel fires (>0 pixel surrounded by <= 0 pixels) contained within each DataArray as a dict.

        Returns:
            dict_proportions: date ('YYYYMM'): proportion of single pixel fire elements
        ...
        # Apply convolutional kernel to binarized data
        for dataArray in self.data:
            data_binary = xr.where(dataArray > 0, 1, 0)
            data = data_binary[0, :, :]
            c = convolve(data, kernel)

            one_pixel__fire_count = data.where(c == 0).sum().values
            dict_proportions[str(dataArray.date.values)] = (
                one_pixel__fire_count / total_count
            )

        return dict_proportions

    def get_burn_area_excluding_single_pixel_fire(self):
        """Returns the proportion of burned area excluding single-pixel fires (>0 pixel surrounded by <= 0 pixels) contained within each DataArray as a dict.

        Returns:
            dict_proportions: date ('YYYYMM'): proportion of burned area excluding single pixel fire elements
        ...
        # Calc burn area excluding single pixel fires
        burn_area = self.get_non_zero_proportion()
        single_fires = self.get_single_pixel_fire_proportion()
        burn_proportion_excl_single = {
            key: burn_area[key] - single_fires.get(key, 0) for key in burn_area
        }

        return burn_proportion_excl_single

    def get_proportion_of_cloudcover(self):
        """Returns the proportion of cloud cover contained within each DataArray as a dict.

        Returns:
            dict_proportions: date ('YYYYMM'): proportion of cloud cover area
        ...
        # Write the proportion of cloud cover (-1 elements) to the output dict
        for dataArray in self.data:
            data_mask = xr.where(dataArray == -1, 1, 0)
            cloud_count = int(data_mask.sum().values)
            dict_proportions[str(dataArray.date.values)] = cloud_count / total_count
    ...
    def plot_hist_for_given_proportion(
        self, dict_proportions, proportion_name, plot_output_path
    ):
        """Plots the means/standard deviations of the given proportion for each month.

        Args:
            dict_proportions: a dict of 'YYYYMM': proportions
            proportion_name: name of proportion
            plot_output_path: path of output file to be saved
        ...
        # Call the corresponding function written in the plotting script
        plotting.monthly_histogram(dict_proportions, proportion_name, plot_output_path)
