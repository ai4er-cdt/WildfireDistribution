def crop_data_spatially(input_data, shape_file_path, zero_remap = 0):
    ...
    try:
        if not ((isinstance(zero_remap, (int, np.integer)) and isinstance(burn_days.values[0,0,0], (int, np.integer))) or
                 (isinstance(zero_remap, (float, np.float64)) and isinstance(burn_days.values[0,0,0], (float, np.float64)))):
            raise TypeError('The zero_remap value provided must match the type of the input data:', type(zero_remap), type(burn_days.values.dtype)) 
